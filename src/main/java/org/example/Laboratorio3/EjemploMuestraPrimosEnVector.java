package org.example.Laboratorio3;
...
// ===========================================================================
  ...
  public static void main( String args[] ) {
    ...
    // (A) ....
    MiHebraPrimoDistCiclica hC[] = new MiHebraPrimoDistCiclica[numHebras];
    for (int i = 0; i < numHebras; i++){
      hC[i] = new MiHebraPrimoDistCiclica(i, numHebras, vectorTrabajo);
      hC[i].start();
    }
    ...
    // Gestion de hebras para la implementacion paralela ciclica
    // (A) ....
    MiHebraPrimoDistPorBloques hB[] = new MiHebraPrimoDistPorBloques[numHebras];
    for (int i = 0; i < numHebras; i++){
      hB[i] = new MiHebraPrimoDistPorBloques(i, numHebras, vectorTrabajo);
      hB[i].start();
    }
    ...
    MiHebraPrimoDistDinamica hD[] = new MiHebraPrimoDistDinamica[numHebras];

    for (int i = 0; i < numHebras; i++){
      hD[i] = new MiHebraPrimoDistDinamica(vectorTrabajo, num);
      hD[i].start();
    }
    ...
    System.out.println( "Tiempo paralela dinámica (seg.):              " + td );
    System.out.println( "Incremento paralela por dinámica:                 " + ts/td );
...
class MiHebraPrimoDistCiclica extends Thread{
  ...
  int numHebras;
  long vectorTrabajo[];
  public MiHebraPrimoDistCiclica(int miId, int numHebras, long[] vectorTrabajo){
    this.miId = miId;
    this.numHebras = numHebras;
    this.vectorTrabajo = vectorTrabajo;
  }
  public void run(){
    for( int i = miId; i < vectorTrabajo.length; i += numHebras ) {
      if( EjemploMuestraPrimosEnVector.esPrimo( vectorTrabajo[ i ] ) ) {
        System.out.println( "  Encontrado primo: " + vectorTrabajo[ i ] );
      }
    }
  }
...
class MiHebraPrimoDistPorBloques extends Thread{
  ...
  int numHebras;
  long vectorTrabajo[];
  public MiHebraPrimoDistPorBloques(int miId, int numHebras, long[] vectorTrabajo){
    this.miId = miId;
    this.numHebras = numHebras;
    this.vectorTrabajo = vectorTrabajo;
  }
  public void run(){
    ...
    int inicio = tam*miId;
    int fin = min(inicio + tam, vectorTrabajo.length);
...
class MiHebraPrimoDistDinamica extends Thread{
  ...
  AtomicInteger num;
  int post;
  public MiHebraPrimoDistDinamica(long[] vectorTrabajo, AtomicInteger num){
    this.vectorTrabajo = vectorTrabajo;
    this.num = num;
  }
  public void run(){
