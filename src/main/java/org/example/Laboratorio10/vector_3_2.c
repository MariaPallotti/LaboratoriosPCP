...
int main( int argc, char *argv[] ) {
  ...
  double  sumaInicial, sumaLocal, sumaFinal, sumaAux;
  ...
  if ( miId == 0 ){
    for( i = 1 ; i < numProcs ; i++){
      MPI_Send( &vectorInicial[i * dimVectorLocal], dimVectorLocal, MPI_DOUBLE, i, 22, MPI_COMM_WORLD);
    }
    for(int i = 0; i < dimVectorLocal; i++){
        vectorLocal[i] = vectorInicial[i];
    }
  }else{
     MPI_Recv( vectorLocal, dimVectorLocal, MPI_DOUBLE, 0, 22, MPI_COMM_WORLD, &st);
  }
  ...
  if ( miId == 0 ){
      sumaFinal = sumaLocal;
      for( i = 1 ; i < numProcs ; i++){
        MPI_Recv( &sumaAux, 1, MPI_DOUBLE, i, 22, MPI_COMM_WORLD, &st);
        sumaFinal += sumaAux;
      }
    }else{
      MPI_Send( &sumaLocal, 1, MPI_DOUBLE, 0, 22, MPI_COMM_WORLD);
    }
